...
def normalize_text(text):
    """
    Минимальная нормализация согласно требованиям:
    - Нижний регистр
    - Удаление пунктуации
    - Сохранение цифр и латиницы (можно конвертировать в русский текст)
    """
    text = text.lower()
    text = text.replace('ё', 'е')  # Нормализация ё -> е
    
    # Преобразование цифр в текст (опционально)
    digit_dict = {
        '0': 'ноль', '1': 'один', '2': 'два', '3': 'три', '4': 'четыре',
        '5': 'пять', '6': 'шесть', '7': 'семь', '8': 'восемь', '9': 'девять'
    }
    
    for digit, word in digit_dict.items():
        text = text.replace(digit, f' {word} ')
    
    # Простая транслитерация латиницы (опционально, только для распространенных случаев)
    # Можно отключить, если в данных нет латиницы
    text = re.sub(r'[a-z]+', lambda m: transliterate_word(m.group()), text)
...
def transliterate_word(word):
    """Базовая транслитерация английских слов"""
    # Простая транслитерация по буквам (можно улучшить)
    translit_map = {
        'a': 'а', 'b': 'б', 'c': 'к', 'd': 'д', 'e': 'е',
        'f': 'ф', 'g': 'г', 'h': 'х', 'i': 'и', 'j': 'дж',
        'k': 'к', 'l': 'л', 'm': 'м', 'n': 'н', 'o': 'о',
        'p': 'п', 'q': 'к', 'r': 'р', 's': 'с', 't': 'т',
        'u': 'у', 'v': 'в', 'w': 'в', 'x': 'кс', 'y': 'ай',
        'z': 'з'
    }
    return ''.join(translit_map.get(c, c) for c in word.lower())